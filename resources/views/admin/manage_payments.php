<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 animate-fade-in">
    <div>
        <h1 class="text-3xl font-extrabold text-foreground">Configuración de Pagos</h1>
        <p class="text-muted-foreground mt-1">Gestiona los datos de los métodos de pago</p>
    </div>
...
<div class="bg-green-500/10 border-l-4 border-green-500 p-4 mb-6 rounded-r-xl">
    <div class="flex items-center">
        ...
        <p class="text-green-500 font-medium">Configuración actualizada correctamente</p>
    </div>
</div>
<?php endif; ?>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-card rounded-2xl shadow-lg border border-border overflow-hidden">
        <div class="bg-gradient-to-r from-primary to-accent px-6 py-5">
            <div class="flex items-center text-primary-foreground">
                ...
                <div>
                    ...
                    <p class="text-primary-foreground/80 text-sm">Datos bancarios Venezuela</p>
                </div>
            </div>
        </div>
        <form action="./admin/payments/update" method="POST" class="p-6 space-y-5">
            ...
            <div>
                <label class="block text-sm font-bold text-foreground mb-2">Teléfono</label>
                <input 
                    type="text" 
                    name="telefono" 
            ...
            <div>
            <div>
                <label class="block text-sm font-bold text-foreground mb-2">Cédula</label>
                <input 
                    type="text" 
                    name="cedula" 
            ...
            <div>
                <label class="block text-sm font-bold text-foreground mb-2">Nombre del Titular</label>
                <input 
                    type="text" 
                    name="titular" 
            ...
            <button type="submit" class="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-3 px-6 rounded-xl transition shadow-lg shadow-primary/25">
        ...
        <div class="bg-gradient-to-r from-yellow-500 to-amber-600 px-6 py-5">
            <div class="flex items-center text-white">
                ...
                <div>
                    ...
                    <p class="text-white/80 text-sm">Pagos en criptomonedas</p>
                </div>
            </div>
        </div>
        <form action="./admin/payments/update" method="POST" class="p-6 space-y-5">
            ...
            <div>
            <div>
                <label class="block text-sm font-bold text-foreground mb-2">Merchant ID</label>
                <input 
                    type="text" 
                    name="merchant_id" 
                    ...
                    class="w-full px-4 py-3 bg-input border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary text-foreground"
                >
                <p class="text-xs text-muted-foreground mt-1">Obtén tu Merchant ID desde el panel de Binance Pay</p>
            </div>

            <div>
                <label class="block text-sm font-bold text-foreground mb-2">API Key (Opcional)</label>
                <input 
                    type="text" 
                    name="api_key" 
                    ...
                    class="w-full px-4 py-3 bg-input border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary text-foreground"
                >
                <p class="text-xs text-muted-foreground mt-1">Solo necesario para integración automática</p>
            </div>

            <div>
                <label class="block text-sm font-bold text-foreground mb-2">Instrucciones para el Cliente</label>
                <textarea 
                    name="instrucciones" 
                    rows="4"
                    class="w-full px-4 py-3 bg-input border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-primary text-foreground"
            ...
            <button type="submit" class="w-full bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3 px-6 rounded-xl transition shadow-lg shadow-yellow-500/25">
...
<div class="mt-6 bg-destructive/10 border-l-4 border-destructive p-4 rounded-r-xl">
    <div class="flex items-start">
        <svg class="w-5 h-5 text-destructive mr-3 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
